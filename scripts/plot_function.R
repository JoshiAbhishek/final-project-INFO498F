...
make_line_plot <- function(df, cause, location) {
  title_plot <-
    paste0("Number of Deaths by ", cause, " from 1999-2013 in ", location)
...
make_age_adjusted_plot <- function(df, cause, location) {
  title_plot <-
    paste0("Age-Adjusted Death Rates for ",
           cause,
           ": ",
           location,
           " (1999-2013)")
  ...
  p <-
    ...
    layout(
      title = title_plot,
      yaxis = list(title = "Rate per 100,000 U.S. standard population")
    )
  return(p)
}
