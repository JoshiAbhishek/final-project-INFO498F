...
causes <- list(
  ...
  "Chronic Lower Respiratory Disease (CLRD)" = 'CLRD',
  "Diabetes" = 'Diabetes',
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      mainPanel(
        ...
        p(
          "This informatics (INFO 498F) project is based off of a dataset of death rates for the top
          leading causes of death in the United States, including mortality patterns from 1999 through
          2013, and by state of residence for selected year and cause of death."
        ),
        p(
          "The data was collected and published by the CDC (Centers for Disease Control and Prevention)
          and accessed through data.gov."
        ),
        
        p(
          "The dataset can be found through this ",
          a(
            "data.gov",
            href = "http://catalog.data.gov/dataset/age-adjusted-death-rates-for-the-top-10
            -leading-causes-of-death-united-states-2013/resource/0e603f1d-31bf-4809-8f10-a994b305b379"
          ),
          " link."
          ),
        h1("Our analysis"),
        ...
        p(
          "We chose to answer these questions with multiple data visualizations.
          To summarize our findings, we found that the leading causes of deaths in total from 1999 to 2013
          included Cancer (8.5M) and Disease of Heart (9.7M), with Pneumonitis (256.3k) and Homicide (261.7k)
          as the lowest."
        ),
        ...
        p(
          "From 1999 to 2013, Alzeihmer's Disease grew the most with 40,231 more deaths in 2013 than 1999;
          Diseases of Heart declined the most with 114,087 deaths less in 2013 than 1999. "
        ),
        p(
        ...
        p(
          "During 1999, Cancer and Diseases of Heart accounted for over 60% of deaths - 27.1% and
          35.8% respectively. During 2013, both causes still accounted for over 57% of deaths - 28.2%
    ...
    tabPanel(
      "Growth By State & Cause",
      sidebarLayout(
        sidebarPanel(
          h4("View the mortality rate growth or decline by state and cause"),
          # Widget 1: Select box for death cause
          selectInput(
            "cause",
            label = h3("Choose a cause of death:"),
            choices = causes,
            selected = 'All Causes'
          ),
    ...
    tabPanel("Causes By Year",
             sidebarLayout(
               sidebarPanel(
                 h4("View the mortality rate for a given year in the U.S."),
                 selectInput(
                   "year2",
    ...
    tabPanel("Totals by state",
             sidebarLayout(
               ...
               mainPanel(plotlyOutput("state_bar"))
             ))
  )
)
